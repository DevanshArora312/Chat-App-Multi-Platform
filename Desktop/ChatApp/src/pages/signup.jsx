                ...
                <div className='w-[25%] min-w-[500px] aspect-[9/10] bg-white/10 rounded-md flex flex-col items-center justify-center gap-4 max-sm:min-w-[250px] max-md:p-5'>
                    ...
                    <form onSubmit={submitHandler} className='flex flex-col items-center justify-center gap-4 p-5'>
                        <div className='w-full'>
                            <input className='rounded-md w-full h-10 bg-white focus:outline-none text-black px-2 ' name="name" id="name" type='text' placeholder='Username' onChange={changeHandler} value={formData.name}></input>
                        </div>
                        <div className='w-full'>
                            <input className='rounded-md w-full h-10 bg-white focus:outline-none text-black px-2 ' name="email" id="email" type='email' placeholder='Email' onChange={changeHandler} value={formData.email}></input>
                        </div>
                        <div className='w-full'>
                            <input className='rounded-md w-full h-10 bg-white focus:outline-none text-black px-2' name="password" id="password" type='password' placeholder='Password' onChange={changeHandler} value={formData.password}></input>
                        </div>
                        <div className='w-full'>
                            <input type='file' ref={fileRef} className='rounded-md w-full h-10 bg-white focus:outline-none text-black px-2' name="dp" id="dp" value={formData.dp} onChange={()=>{
                                setFormData(prev =>{
                                    return{
                                        ...prev,
